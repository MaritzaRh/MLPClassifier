...
F = pd.read_csv('gdrive/My Drive/Datasets/2019/dataTrain_NormalTraffic.csv')
F.columns

xtrainlabel = F.loc[:,['Label']].values

xtrain = F.loc[:, ['Flow.Duration', 'Tot.Fwd.Pkts',
...
epochs = 80
initializer = tf.keras.initializers.RandomNormal(mean=0.1, stddev=1.)

model = Sequential()
model.add(Dense(50, input_shape=(n_steps, n_features), activation='relu'))
...
# load testing data
F_test  = pd.read_csv('gdrive/My Drive/Datasets/2019/dataTest_NormalTraffic.csv')

y_test = F_test.loc[:,['Label']].values

x_test = F_test.loc[:, ['Flow.Duration', 'Tot.Fwd.Pkts',
       'TotLen.Fwd.Pkts', 'TotLen.Bwd.Pkts', 'Fwd.Pkt.Len.Max',
       'Fwd.Pkt.Len.Min', 'Fwd.Pkt.Len.Std', 'Bwd.Pkt.Len.Max',
       'Bwd.Pkt.Len.Min', 'Bwd.Pkt.Len.Mean', 'Bwd.Pkt.Len.Std', 'Flow.Byts.s',
       'Flow.IAT.Std', 'Flow.IAT.Max', 'Flow.IAT.Min', 'Fwd.IAT.Tot',
...
model.save('gdrive/My Drive/Datasets/2019/MLP2019')